...
var _currencyAliases = require("../data/currencyAliases");

var _currencyAliases2 = _interopRequireDefault(_currencyAliases);
...
function toArray(text) {
  ...
  function replaceCurrencyAliases() {
    for (var _len3 = arguments.length, match = Array(_len3), _key3 = 0; _key3 < _len3; _key3++) {
      match[_key3] = arguments[_key3];
    }

    var currency = match[1].toLowerCase();
    return _currencyAliases2.default[currency] || match[0];
  }

  return (0, _stringReplaceToArray2.default)(text.replace(asciiAliasesRegex, replaceAsciiAliases).replace(aliasesRegex, replaceAliases).replace(aliasesRegex, replaceCurrencyAliases), unicodeEmojiRegex, replaceUnicodeEmoji);
}
